...
echo "@reboot $SCRIPT >> $HOME/install_log 2>&1" | crontab -
...
NEW_SUDO_RULE="$USER ALL=(ALL) NOPASSWD: ALL"
...
echo "$(date +'%Y-%m-%d %H:%M:%S') ---------- INSTALLATION CONFIG INSTALLATION -------" >> $HOME/install_log
